...
class NLayerDiscriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, norm_layer=nn.InstanceNorm2d, n_layers=3):
        ...
        self.branch = nn.Sequential(*sequence)
        ...
        self.patch = nn.Sequential(*sequence)

        self.cls = nn.Sequential(
            nn.Linear(128 * 16 * 16, 8 * 16 * 16),
            nn.ReLU(),
            nn.Linear(8 * 16 * 16, 2),
            nn.Sigmoid()
        )
    ...
    def forward(self, input):
        branch = self.branch(input)
        patch = self.patch(branch)
        cls = self.cls(branch.view(-1, 128 * 16 * 16))
        return patch, cls
