...
class tVAE_tf(object):
    ...
    def _train_graph(self):
        ...
        aux_loss = self._aux_loss(mu, logvar)
    ...
    def _train_loss(self, kld_loss, rec_loss, aux_loss):
        with tf.name_scope('loss'):
            loss = rec_loss + self.flags.beta * self._kld_coef() * kld_loss + aux_loss
        return loss
    ...
    def _aux_loss(self, mu, logvar):
        with tf.name_scope('aux_loss'):
            ...
            aux_loss = -0.5 * tf.reduce_mean(tf.reduce_sum(logvar, 1))
        return aux_loss
