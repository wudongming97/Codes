...
def next_batch_(bz, train=True):
    while True:
        data = fashion.train.next_batch(bz) if train else fashion.test.next_batch(bz, shuffle=True)
        yield np.reshape(data[0], [bz, 28, 28, 1]), data[1]
