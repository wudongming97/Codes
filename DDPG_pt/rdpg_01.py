...
act_net_t = RDPG_Actor(obs_size, act_size)
...
replay_buffer = EpisodeBuffer(1000, EPISODE_STEP)
act_trainer = optim.Adam(act_net.parameters(), lr=1e-4)
cri_trainer = optim.Adam(cri_net.parameters(), lr=1e-3)


def rdpg_update(batch_size):
    hx, hx_t = None, None
    loss_act, loss_cri = 0, 0
    for t in range(EPISODE_STEP):
        state, action, reward, next_state = to_tensors(replay_buffer.sample(batch_size, t))
        pred_act, hx = act_net(state, hx)
        loss_act -= cri_net(state, pred_act).sum(1).mean()
...
while True:
    ...
    for t in range(EPISODE_STEP):
        action, hx = act_net.get_action([state], hx)
    ...
    loss_act, loss_cri = rdpg_update(batch_size)
