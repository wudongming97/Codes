...
class DataLoader:
    ...
    def unpack_for_mixvare(self, batch_tensor, seq_len):
        sorted_tensor = sorted(batch_tensor, key=len, reverse=True)
        X_i, X_lengths = self._pad([[self.vocab.idx[G_TOKEN]] + line for line in sorted_tensor], seq_len)
        X_t, _ = self._pad([line + [self.vocab.idx[E_TOKEN]] for line in sorted_tensor], seq_len)
        return X_i, X_lengths, X_t, self._mask(X_lengths, seq_len)
