...
class VAE_(models.VAE_base.VAE):
    ...
    def _encoder(self, X, ru=False):
        with tf.variable_scope('mlp_encoder', reuse=ru):
            ...
            fc2 = tf.layers.dense(fc1, 512, activation=tf.nn.elu, kernel_initializer=self.initializer, name='L2')
    ...
    def _decoder(self, z, ru=False):
        with tf.variable_scope('mlp_decoder', reuse=ru):
            dc0 = tf.layers.dense(z, 32, activation=tf.nn.elu, kernel_initializer=self.initializer, name='D0')
            dc1 = tf.layers.dense(dc0, 512, activation=tf.nn.elu, kernel_initializer=self.initializer, name='D1')
            dc2 = tf.layers.dense(dc1, 512, activation=tf.nn.elu, kernel_initializer=self.initializer, name='D2')
            dc3 = tf.layers.dense(dc2, 784, kernel_initializer=self.initializer, name='D4')
